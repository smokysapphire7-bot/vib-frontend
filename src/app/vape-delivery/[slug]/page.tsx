...
function getSeoContent(name: string): string {
  ...
  if (["hospital","health","clinic","apollo","manipal","fortis","narayana","sakra","aster","columbia","cloudnine","vikram","bgs"].some(x => n.includes(x))) {
    return `Vape delivery near ${name} in Bangalore is available 7 days a week. Whether you are visiting a patient, working nearby, or living in the area — we deliver Elfbar disposables, Caliburn pod systems, ZYN nicotine pouches and more in 30-45 minutes. All products are 100% original, sealed, and delivered discreetly.`;
  }
  if (["tech park","business park","manyata","bagmane","embassy","prestige","itpl","cessna","rmz","ecospace","outer ring road"].some(x => n.includes(x))) {
    return `Working at ${name} in Bangalore? We deliver vapes to your office gate or nearby address in 30-45 minutes. Elfbar Raya D1, Lost Mary MT35000, Caliburn G4, ZYN Cool Mint and more. Order on WhatsApp and we dispatch immediately.`;
  }
  if (["college","university","iisc","christ","rv college","bms","pesit","jain","ramaiah"].some(x => n.includes(x))) {
    return `Near ${name} in Bangalore? We deliver to the campus area in 30-45 minutes. Elfbar 600 from Rs 1,199, Elfbar Raya D1 Rs 2,399, ZYN Cool Mint Rs 1,299 and more. 100% original products, fast and discreet.`;
  }
  if (["mall","phoenix","orion","forum","mantri","garuda","ub city"].some(x => n.includes(x))) {
    return `Near ${name} in Bangalore? We deliver vapes to ${name} and all surrounding areas in 30-45 minutes. Browse our full range of Elfbar disposables, Caliburn pod systems, ZYN and Velo nicotine pouches.`;
  }
  if (["leela","itc","taj","marriott","oberoi","four seasons","sheraton","st regis","hyatt"].some(x => n.includes(x))) {
    return `Staying at ${name} in Bangalore? We deliver premium vapes discreetly in 30-45 minutes. Elfbar Raya D1, MoonNight 40K, Caliburn G4, ZYN Cool Mint — all 100% original. Just message us on WhatsApp.`;
  }
...
const WA = "https://wa.me/919074445985?text=Hi%20Vape Delivery Bangalore%2C%20I%20want%20to%20order%20a%20vape%20in%20Bangalore";
  ...
  return (
    <main style={{ minHeight: "100vh", background: "var(--bg)", paddingBottom: 80 }}>
        ...
        <div style={{ background: "var(--bg-2)", border: "1px solid var(--border)", borderRadius: 14, padding: "20px", marginBottom: 24 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column" as const, gap: 8 }}>
            {[
              ...
              "No COD — payment confirmed on WhatsApp before dispatch",
              "Delivery charge paid to rider on arrival",
            ].map(f => (
      ...
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({
        ...
        "@type": "FAQPage",
        "mainEntity": [
          { "@type": "Question", "name": `Is vape delivery available in ${loc.name}?`, "acceptedAnswer": { "@type": "Answer", "text": `Yes — we deliver vapes to ${loc.name}, Bangalore in 30-45 minutes.` } },
          { "@type": "Question", "name": `What is the price of vapes in ${loc.name}?`, "acceptedAnswer": { "@type": "Answer", "text": `Vapes in ${loc.name} start from Rs 1,199. Elfbar Raya D1 is Rs 2,399, ZYN Cool Mint Rs 1,299.` } },
        ]
      })}} />
    </main>
  );
